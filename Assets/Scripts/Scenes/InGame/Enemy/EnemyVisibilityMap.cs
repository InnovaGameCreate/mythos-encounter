...
namespace Scenes.Ingame.Enemy
{
    public class EnemyVisibilityMap : MonoBehaviour
    {
        ...
        public struct DoubleByteAndMonoFloat {//位置と距離
            public byte x;
            public byte z;
            public float range;
        ...
        public struct VisivilityArea {//各エリアの情報、そこを見ることのできる他の位置とこのエリアを見た回数
        ...
        public void GridMake(byte x,byte z,float range ,Vector3 setCenterPosition) { //マップを作成。xとzはグリッドの配置数。rangeはグリッドの距離。centerPositionは左下の位置
            visivilityAreaGrid = new List<List<VisivilityArea>>();
            gridRange = range;
            centerPosition =  setCenterPosition;
            for (byte i = 0; i < x; i++) { //配列の要素を作成
        ...
        public void MapScan() {//マップをスキャンして実際の視界がどのように通っているかを設定
            //各マス目へとアクセス
            for (byte x  = 0;x < visivilityAreaGrid.Count();x++) {
                for (byte z = 0; z < visivilityAreaGrid[0].Count(); z++)
                {
                    ...
                    for (byte vX = 0; vX < visivilityAreaGrid.Count(); vX++) {
                        ...
                        {
                            if ((x!= vX) || (z != vZ)) { //自分自身ではない場合                               
                                float range2 = Mathf.Pow((x - vX) * 5.8f,2) + Mathf.Pow((z - vZ) * 5.8f, 2);
                                if (range2 <= Mathf.Pow(maxVisivilityRange,2)) { //視界が通るとされる距離でない場合
