...
namespace Scenes.Ingame.Stage
{
    ...
    public class Multi_StageGenerator : NetworkBehaviour
    {
        ...
        void Start()
        {
            ...
            IngameManager.Instance.OnInitial    //初期化フラグが立ったら
                .Subscribe(_ =>
                {
        ...
        private async UniTaskVoid Generate(CancellationToken token)
        {
            ...
            for (int floor = 1; floor <= 2; floor++)
            {
                ...
                RoomPlotId(RoomType.room3x3Stair, new Vector2((int)_stageSize.x - 3, (int)_stageSize.y - 3), targetFloor);  //確定の階段部屋データの入力　(階段の位置は固定？)
            ...
            await StairRoomSelect(_firstFloorData, _secondFloorData);                           //階段の位置の選択
        ...
        void OnStageDataReceived(NetworkRunner runner, PlayerRef player, ReliableKey key, ArraySegment<byte> data)
        {
            ...
            if(keys[0] == 1)    //_firstFloorを受け取った時
            {
        ...
        {
            playerSpawnRoom = false;    //プレイヤーのスポーン位置があるかどうかのフラグ?
        ...
        private async UniTask GenerateStage(CancellationToken token, RoomData[,] stage, int floor)
        {
            ...
            Debug.Log("flasの長さ:" + roomFlag.Length);
