...
namespace FischlWorks_FogWar
{
    ...
    public class csFogWar : MonoBehaviour
    {
        ...
        [SerializeField] private int myFloor;
        ...
        private void Start()
        {
            IngameManager.Instance.OnStageGenerateEvent
                ...
                .Subscribe(_ =>
                {
                    ...
                    InitializeFog();
                    FindObjectOfType<Scenes.Ingame.Player.Map>().SetFogs(myFloor, fogPlane.GetComponent<MeshRenderer>());
        ...
        private void InitializeFog()
        {
            ...
            fogPlane.layer = LayerMask.NameToLayer("MapOnly");
        }
        ...
        private void UpdateFogField()
        {
            ...
            foreach (FogRevealer fogRevealer in fogRevealers)
            {
                if (!isFirst && myFloor == 1 && fogRevealer._RevealerTransform.position.y > 5.8f)
                {
                    return;
                }

                if (!isFirst && myFloor == 2 && fogRevealer._RevealerTransform.position.y <= 5.8f)
                {
                    return;
                }
